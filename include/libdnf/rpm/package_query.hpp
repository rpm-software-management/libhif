...
public:
    ...
    /// Filter packages by their `name-[epoch:]version-release.arch`. The following matches are tolerant to omited 0 epoch: `EQ`, `NEQ`, `GT`, `GTE`, `LT`, `LTE`.
    ///
    /// @param patterns         A vector of strings the filter is matched against.
    /// @param cmp              A comparison (match) operator, defaults to `QueryCmp::EQ`.
    ...
    /// Filter packages by grouping them by `name` and `arch` and keeping up to `limit` number of them from each group.
    ///
    /// @param limit            If `limit` > 0, keep `limit` number packages in each group.
    ///                         If `limit` < 0, keep all **but** `limit` last packages in each group.
    /// @since 5.0
    //
    ...
    /// Filter packages by grouping them by `name` and `arch` and keeping those that belong to a repo with the highest priority (the lowest number) from each group.
